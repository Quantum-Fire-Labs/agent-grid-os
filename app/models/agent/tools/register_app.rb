class Agent::Tools::RegisterApp < Agent::Tools::Base
  def self.definition
    {
      ...
      function: {
        ...
        parameters: {
          type: "object",
          properties: {
            name: { type: "string", description: "App name (lowercase letters, numbers, and hyphens, e.g. 'slideshow' or 'task-board')" },
          ...
          required: %w[name description path]
        }
      }
    }
  end
  ...
  def call
    ...
    custom_app = agent.custom_apps.find_or_initialize_by(name: name)
    ...
    if custom_app.save
      "Registered app '#{name}'. Users can access it at /apps/#{custom_app.id}."
    else
