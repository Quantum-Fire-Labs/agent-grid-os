...
class Providers::OpenAi < Providers::Client
  HOST = "api.openai.com"
  PATH = "/v1/chat/completions"

  MODELS_PATH = "/v1/models"

  def self.display_name = "OpenAI API"

  def self.models(key_chain)
    return [] unless key_chain&.api_key

    Rails.cache.fetch("provider_models/openai", expires_in: 5.minutes) do
      ...
      (data["data"] || []).filter_map { |m|
        next unless m["id"].start_with?("gpt-")
        { id: m["id"], name: m["id"] }
      }.sort_by { |m| m[:id] }
    end
  ...
  def chat(messages:, model: nil, tools: nil, &on_token)
    ...
    payload[:stream_options] = { include_usage: true } if on_token
